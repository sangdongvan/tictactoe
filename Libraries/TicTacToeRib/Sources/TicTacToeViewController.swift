...
public protocol TicTacToePresentableListener: class {
    func placeCurrentPlayerMark(atRow row: Int, col: Int)
}

public final class TicTacToeViewController: UIViewController, TicTacToePresentable,
    TicTacToeViewControllable
{

    weak public var listener: TicTacToePresentableListener?
    ...
    public func setCell(atRow row: Int, col: Int, withPlayerType playerType: PlayerType) {
        ...
        let cell = collectionView.cellForItem(
            at: IndexPath(row: indexPathRow, section: Constants.sectionCount - 1))
        cell?.backgroundColor = playerType.color
    }

    public func announce(winner: PlayerType?, withCompletionHandler handler: @escaping () -> Void) {
        ...
        alert = UIAlertController(title: winnerString, message: nil, preferredStyle: .alert)
        let closeAction = UIAlertAction(title: "Close Game", style: UIAlertAction.Style.default) {
            _ in
            handler()
        }
        if let alert = alert {
    ...
    /// alert instance is kept for testing purpose, you should not use this property.
    ...
    private func buildCollectionView() {
        ...
        collectionView.register(
            UICollectionViewCell.self, forCellWithReuseIdentifier: Constants.cellIdentifier)
        view.addSubview(collectionView)
        collectionView.snp.makeConstraints { (maker: ConstraintMaker) in
            maker.center.equalTo(self.view.snp.center)
            maker.size.equalTo(
                CGSize(
                    width: CGFloat(GameConstants.colCount) * Constants.cellSize,
...
extension TicTacToeViewController: UICollectionViewDataSource {
    ...
    public func collectionView(
        ...
        let reusedCell = collectionView.dequeueReusableCell(
